...
// import {COL_SIZE, ROW_SIZE} from './shape';
...
// class Tetris extends Component<IProps, IState> {
// 	state = {
...
//   // shift
//   shiftRight = (isRight) => {
//     const curShape = s.getShape(this.state);
...
//       const newArray  = oldArray.filter(val => val !== s.DEFAULT_VALUE);
...
//       const row = [...Array(ROW_SIZE)].map( (_, pos) => pos + ROW_SIZE * (this.state.yPos + i));

//       // getting the value of all the bottom elements
//       const isFilled = row.map( pos => this.state.board[pos])
...
//       const newArray = curShape.map(row => (row[pos]=== s.DEFAULT_VALUE)? -1: row[pos] + ROW_SIZE);
//       const bottomValue = Math.max.apply(Math, newArray);
//       if (
